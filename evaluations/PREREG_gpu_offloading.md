<!-- STATUS: ENTSCHIEDEN | Frage: Laesst sich die Inferenz von der CPU auf die GPU verlagern -- erreicht Verschraenkung vieler gleichzeitiger Partien den Batch, an dem die GPU gewinnt? | Beleg: NEIN, geschlossen bis zum groesseren Netz (Verdikt 2026-08-14). Weg V wurde gebaut und vermessen: die Batch-Erwartung ist ERFUELLT (Fuellung 99,7 % bei N=128), der Durchsatz aber VERFEHLT (beste Zelle 1,255x). Der Batch war nie der Engpass, der Amdahl-Rest ist es. Herleitung und Verdikt im Koerper. -->
...
## VERDIKT: ENTSCHIEDEN (Verdikt 2026-08-14, hier nachgetragen 2026-08-21; aus dem Statuskopf in den Koerper gezogen 2026-08-28)

Weg V wurde umgesetzt (`PREREG_async_search.md` Stufe 1-3, Archiv-Branches
`async_search_stage{1,3}_archive`) und in `PREREG_gpu_inference_path.md`
§20-§23 vermessen.

| Groesse | Ergebnis |
|---|---|
| Batch-Erwartung (Regel 1) | **ERFUELLT** -- Fuellung 99,7 % bei N=128 |
| Durchsatz (Regel 3) | **VERFEHLT** -- beste Zelle 1,255x gegen den ehrlichen Sync-Nenner 528,5 Partien/h |

**Die GPU-Verlagerung ist damit GESCHLOSSEN, bis ein groesseres Netz die
Kennlinie verschiebt.** Der Batch war nie das Problem; der Amdahl-Rest ist es.

### STALE-FALLE 2026-08-12 (Lehre zum Statuskopf selbst)

Diese Kopfzeile trug bis zum 2026-08-12 noch "offen ist die
Blatt-Erzeugungsrate" und hat genau dadurch einen Agenten-Auftrag ausgeloest,
der die Zahl neu messen sollte -- sie lag seit `e1bce64` vor. **Ein veralteter
Index kostet Arbeit, nicht nur Klarheit.**

**Artefakt:** `evaluations/artifacts/interleave_batch_probe.json` (enthaelt nur
die Teil-1-Daten).
