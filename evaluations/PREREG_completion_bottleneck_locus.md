<!-- STATUS: OFFEN | Frage: Wirkt der Implicit-Minimax-Effekt UPSTREAM auf die Reihenwahl, oder nur auf die Endplatzierung -- sitzt der Vollendungs-Engpass also in der SUCHE oder im ZIEL? | Beleg: nichts gebaut, angelegt 2026-08-23. Anlass: Legalitaets-Stufe 0/160 (Vollendung war nie legal moeglich, 54 % Blockade = Musterreihe nicht voll) plus Implicit-Minimax k1 9,0 auf 16,0 Prozent aus reiner Suchaenderung. Beide Sonden existieren, der Knopf ist abgenommen; der Zuschnitt dockt auf die ohnehin geplante Alpha-Messung (agent_encapsulation par.6a) auf und braucht dann KEINE eigenen Partien -->
...
3. **Eine reine SUCHAENDERUNG hat k1 bewegt.** Implicit-Minimax-Backup:
   k1 von 9,0 % auf 16,0 % (+7,0 pp, p=0,090), bei unveraendertem Netz,
   Ziel und Korpus. k1 ist das Spalten-Kriterium (`docs/knobs.md:61`,
   `MOSAIC_SPALTENBAU` = "Spaltenbauer-Vorzugsschicht (Kriterium 1)").
...
## par.3 Warum das fast nichts kostet
...
**Und die Partien fallen ohnehin an.** `PREREG_agent_encapsulation.md`
par.6a hat die Alpha-Messung bereits vorregistriert (Champion gegen
Champion+alpha=0,2, netz-gegen-netz, 407 Seeds) -- noch nicht gelaufen.
**Dieser Zuschnitt erzeugt KEINE eigenen Partien**, sondern laesst beide
Sonden auf deren Artefakten laufen.

Einzige Voraussetzung: jene Messung muss mit `--log-games` fahren. Steht sie
schon ohne, ist das der einzige Punkt, der vorher zu klaeren ist.
