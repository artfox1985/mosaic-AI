# Spielanalyse: game_rng_schnitt_probe_4_seed8709371129893951522.log

Erzeugt von `tools/analyze_game_log.py` (Commit `a191cd7`), Laufzeit 0s.

- Seed: 8709371129893951522, Startspieler: Netz, KI-Spieler: Netz (alphazero_v21_2d_brierbest, 40 Sims)
## (a) Zusammenfassung

Endstand (aus dem Log-Text): **Netz 80 : 48 Heuristik**

Replay-Kreuzvalidierung: jede einzelne erzeugte Log-Zeile (`log_since`) wurde exakt (String-Gleichheit inkl. `[Rn] `-Präfix) gegen die Original-Logdatei geprüft (alle 315 Zeilen bestehen).
...
- Netz: nicht bewertete Züge -- 43× --no-oracle, 8× Runde 5 -- exakter Alpha-Beta-Solver (round5.rs), nicht netz-oracle-bewertet
- Heuristik: nicht bewertete Züge -- 44× --no-oracle, 9× Runde 5 -- exakter Alpha-Beta-Solver (round5.rs), nicht netz-oracle-bewertet
...
(Rohpunktestand direkt vor der Endwertung, aus der `# SPIELENDE:`-Kopfzeile: Netz 80 : 48 Heuristik -- fehlende "–"-Werte oben bedeuten lediglich 0 Pkt Strafe in dieser Runde, keine Lücke.)

Endwertung (Wertungsplatten-Bonus):

- **Netz**: +17 Pkt -> Gesamt 80 Pkt
  - ↗️ Diagonale Reihen: 0 Pkt
  - 🌈 Mehrfarbige Felder: 10 Pkt
  - ↕️ Vertikale Reihen: 7 Pkt
- **Heuristik**: +8 Pkt -> Gesamt 48 Pkt
  - ↗️ Diagonale Reihen: 0 Pkt
  - 🌈 Mehrfarbige Felder: 8 Pkt
  - ↕️ Vertikale Reihen: 0 Pkt

Vor der Endwertung stand es 80 : 48; nach dem Wertungsplatten-Bonus 80 : 48.
...
- **Entdeckte Logging-Luecke (KI-Bonuschips)**: der Mensch-Pfad `apply_tiling_chips` (py.rs) loggt "🎫 ... komplettiert Reihe N ...", der KI-Pfad (`ai_tiling_step` -> `TilingStep::Chips` -> `apply_bonus_chips_with`, round_end.rs) tut das NICHT. Betroffen in dieser Partie: R2 Heuristik Reihe 3, R2 Heuristik Reihe 4, R3 Netz Reihe 3, R3 Netz Reihe 4, R4 Heuristik Reihe 5, R5 Netz Reihe 3. Das Replay-Werkzeug erkennt die unvollstaendige Zielreihe und holt die Chip-Komplettierung automatisch nach (ohne die dabei entstehende, im Original fehlende "🎫"-Zeile gegen das Log zu pruefen).
