# Spielanalyse: game_rng_schnitt_probe_3_seed15755400384280304653.log

Erzeugt von `tools/analyze_game_log.py` (Commit `a191cd7`), Laufzeit 0s.

- Seed: 15755400384280304653, Startspieler: Heuristik, KI-Spieler: Netz (alphazero_v21_2d_brierbest, 40 Sims)
## (a) Zusammenfassung

Endstand (aus dem Log-Text): **Netz 48 : 31 Heuristik**
...
- Heuristik: nicht bewertete Züge -- 45× --no-oracle, 8× Runde 5 -- exakter Alpha-Beta-Solver (round5.rs), nicht netz-oracle-bewertet
...
(Rohpunktestand direkt vor der Endwertung, aus der `# SPIELENDE:`-Kopfzeile: Netz 48 : 31 Heuristik -- fehlende "–"-Werte oben bedeuten lediglich 0 Pkt Strafe in dieser Runde, keine Lücke.)

Endwertung (Wertungsplatten-Bonus):

- **Netz**: +19 Pkt -> Gesamt 48 Pkt
  - 🌈 Mehrfarbige Felder: 10 Pkt
  - ⬜ Äußere Felder: 9 Pkt
  - ↗️ Diagonale Reihen: 0 Pkt
- **Heuristik**: +8 Pkt -> Gesamt 31 Pkt
  - 🌈 Mehrfarbige Felder: 0 Pkt
  - ⬜ Äußere Felder: 8 Pkt
  - ↗️ Diagonale Reihen: 0 Pkt

Vor der Endwertung stand es 48 : 31; nach dem Wertungsplatten-Bonus 48 : 31.
...
- **Entdeckte Logging-Luecke (KI-Bonuschips)**: der Mensch-Pfad `apply_tiling_chips` (py.rs) loggt "🎫 ... komplettiert Reihe N ...", der KI-Pfad (`ai_tiling_step` -> `TilingStep::Chips` -> `apply_bonus_chips_with`, round_end.rs) tut das NICHT. Betroffen in dieser Partie: R3 Heuristik Reihe 4, R3 Netz Reihe 2, R4 Netz Reihe 5, R5 Heuristik Reihe 3. Das Replay-Werkzeug erkennt die unvollstaendige Zielreihe und holt die Chip-Komplettierung automatisch nach (ohne die dabei entstehende, im Original fehlende "🎫"-Zeile gegen das Log zu pruefen).
