...
| 33 | `round_transition_search_sampling` par.9/par.4.2/par.10 | Bau Variante B als Knopf `MOSAIC_ROUND_TRANSITION_LEAF` (Tiling im Blatt, EINE Neubefuellung, stellungsgebundener Seed, Beutel mit Turm daneben). **GEBAUT 2026-09-16, Baustand in par.17**: Knopf plus Spec-Feld (optional, Default 0), Registratur, Manifest, Blatt-Eingriff in `make_node`, `[rt_leaf]`-Diagnosezeile, 9 neue Tests gruen, `cargo test --release --lib` 672 gruen / 1 rot (die rote ist FREMD und lag schon an HEAD: `MOSAIC_MOON_TARGET_SOURCE` ohne Registratur-Eintrag). **NOCH NICHT im Wheel** -- Wheel-Bau, Anker-Drift/Konservierung und Paritaets-Fixture stehen aus und gehoeren vor Nr. 34 | CPU-freies Fenster, Champion v29-b01 | Bau in EINEM Arbeitsgang erledigt (statt "rund ein Tag"); gemessen davon nur die Kompilier- und Testzeiten: 57,7 s Bau, 99,6 s Testlauf, 42,4 s `--no-run`; Tore rund 6 min (gemessen) | **ja** -- bei ROT |
