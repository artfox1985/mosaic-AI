...
| 29a | `moon_stack_order` par.9, Ergebnis par.9h | A/B Stufe 3: eigene Nachsuche fuer die Reihenfolge NACH der Zugwahl, gegen kanonisch. **DURCH 2026-09-15: traegt NICHT** (197:203, McNemar p 0,84, 200 Paare ohne Frueh-Stopp; keine der sechs Kennzahlen ueber der Aufloesung; 5.933 s, 14,833 s je Partie). Erstmals war der Wert der Reihenfolge ALLEIN gemessen -- Stufe 1 hatte die Kandidatenkonkurrenz im Wurzelfenster mitgemessen. Der Implementierungs-Verdacht aus par.9d ist in 3 von 5 Punkten am Code ausgeraeumt (Perspektive, Verdrahtung bis in den Arena-Pfad, Vollstaendigkeit der Varianten); offen bleiben Budget (256 Sims je Variante = Wurzelbreite 16) und Ausloesungsrate -- beide nur messbar, dafuer ist die Diagnose-Zeile gebaut (par.9g) | Nr. 28, Stufe-3-Bau par.9a | 5.933 s (gemessen) | **ja** -- Bauform und Budget sind Nutzer-Entscheide |
...
| 32b | `moon_stack_order` par.10 Weg B | **Zielwechsel des `moon`-Kopfs**: der Prior kommt heute aus dem Rundenloeser, ist also auf das RUNDENENDE gerichtet statt auf den Partieausgang (par.9e, Nutzer: "Plackett-Luce-Modell ist halt kurzfristig"). Kein neuer Kopf, keine Aktionsraum-Erweiterung -- `feedback_no_new_heads` erlaubt Zielwechsel. Ein Trainingsarm mit eigener v29-bXX-Nummer plus Gating. Lesart: traegt der Zielwechsel, ist der Prior die Ursache; traegt er nicht, bleibt Weg A (eigener Entscheidungsknoten) als einzige Erklaerung | Nr. 32a (C ist der kleinere Eingriff und adressiert denselben Einwand ohne Training) | Trainingsarm plus Gating rund 1,5 h (ANNAHME) | **ja** -- Armnummer und Rezept-Aufnahme |
