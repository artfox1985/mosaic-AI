...
| 29b | `dome_return_order` par.12 | **Architektur konkretisiert (2026-09-15)**: drei Grenzen am Code (kein Fan-out im Baum, Entscheider ausserhalb der Suche mit einem Blattwert, Encoder sieht die eigene Reihenfolge nur als Typfolge). Reihenfolge: R1 Sensitivitaets-Sonde am v29-b03 (Spannweite des Value-Kopfs ueber die Permutationen) -> v30-Erzeugung mit Streu-Knopf 0,015 -> R1 am v30-Netz -> A/B Modus 1 gegen 0 nur bei messbarer Spannweite; R2 (geordnete eigene Designs, +4 Werte) nur wenn die Typfolge der Deckel ist, VOR der Erzeugung ins Record; R3 (Knoten im Baum) nur gebuendelt mit Mondstapel Weg A, ausserhalb des v30-Rahmens | R1: Maschine frei, Netz v29-b03 | R1 Bau rund 1 h (ANNAHME), Lauf Minuten | **ja** -- Modus 1 in der v30-Erzeugung an oder aus; R2 ja/nein vor der Erzeugung |
...
| 32c | `moon_stack_order` par.12.3 | **Zugriffs-Bilanz-Sonde** (Instrument, kein Bau am Spiel): je Mondstapel-Entscheid aus den vorhandenen `--log-games`-Artefakten, WER den oben gelegten Stein als naechstes nimmt (Waehler, Gegner, niemand vor Rundenende) und wie viele Halbzuege spaeter. Lesart vorab: nimmt der Gegner ihn in unter einem Viertel der Faelle im naechsten Halbzug, ist der Hebel klein und weder C1 noch A lohnen; ueber der Haelfte ist der Hebel real. Dazu die Korpus-Sonde aus par.12.1 (Anteil nicht-kanonischer gespielter Reihenfolgen im v29-Fenster). Beide VOR 32b und vor C1 | Logs liegen (`moon_order_post_vs_off_s20261091`, K4-Laeufe), v29-Fenster | Bau rund 1 h (ANNAHME), Lauf Sekunden bis Minuten | nein |
