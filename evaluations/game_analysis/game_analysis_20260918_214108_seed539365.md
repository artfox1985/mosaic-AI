# Spielanalyse: game_20260918_214108_seed539365.log

Erzeugt von `tools/analyze_game_log.py` (Commit `51411437`), Laufzeit 0s.

- Seed: 539365, Startspieler: Spielerin, KI-Spieler: KI (v29-b09_brierbest, 400 Sims)
## (a) Zusammenfassung

Endstand (aus dem Log-Text): **Spielerin 80 : 64 KI**

Replay-Kreuzvalidierung: jede einzelne erzeugte Log-Zeile (`log_since`) wurde exakt (String-Gleichheit inkl. `[Rn] `-Präfix) gegen die Original-Logdatei geprüft (alle 324 Zeilen bestehen).
...
Chip-Wahl aus dem LOG uebernommen: 6 Vollendung(en) (0 davon mit mehreren farbgleichen Kandidaten, dort ist die Wahl fuer Regel und Log ununterscheidbar). Fuer diese Ereignisse entfaellt das Raten samt Backtracking (`chip_plan`, Vorfall 2026-08-29).
...
- Spielerin: nicht bewertete Züge -- 44× --no-oracle, 8× Runde 5 -- exakter Alpha-Beta-Solver (round5.rs), nicht netz-oracle-bewertet
...
(Rohpunktestand direkt vor der Endwertung, aus der `# SPIELENDE:`-Kopfzeile: Spielerin 45 : 54 KI -- fehlende "–"-Werte oben bedeuten lediglich 0 Pkt Strafe in dieser Runde, keine Lücke.)

Endwertung (Wertungsplatten-Bonus):

- **Spielerin**: +35 Pkt -> Gesamt 80 Pkt
  - ↕️ Vertikale Reihen: 14 Pkt
  - 🌈 Mehrfarbige Felder: 10 Pkt
  - 🔲 Eckplatten: 11 Pkt
- **KI**: +10 Pkt -> Gesamt 64 Pkt
  - ↕️ Vertikale Reihen: 7 Pkt
  - 🌈 Mehrfarbige Felder: 0 Pkt
  - 🔲 Eckplatten: 3 Pkt

Vor der Endwertung stand es 45 : 54; nach dem Wertungsplatten-Bonus 80 : 64.
