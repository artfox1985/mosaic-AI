# Spielanalyse: game_20260904_194912_seed771522.log

Erzeugt von `tools/analyze_game_log.py` (Commit `1ca379f`), Laufzeit 0s.

- Seed: 771522, Startspieler: Spieler 1, KI-Spieler: KI (v23-b01_k3p10, 400 Sims)
## (a) Zusammenfassung

Endstand (aus dem Log-Text): **Spieler 1 37 : 36 KI**
...
⚠️ Chip-Zeile: 6 Zeile(n) nur beim Symbol toleriert (altes 🎫 gegen heutiges 🎴), 6 nur beim fehlenden Plaettchen-Zusatz. Beides sind die benannten Toleranzen fuer Logs von vor dem 2026-09-07; die heutige Engine schreibt `🎴 ... (3 Plättchen: rot, ...)`.

⚠️ Rueckleg-Zeile: 1 Zeile(n) nur bei der Plattenliste toleriert (Logs vor dem 2026-09-10 nennen `(Reihenfolge): #id (Typ), ...`, die heutige Engine nur noch die Anzahl -- die Reihenfolge sieht nur der ziehende Spieler). Reihenfolge aus der `#a`-Zeile uebernommen: 2 Stapelzug/-zuege.
...
- Spieler 1: nicht bewertete Züge -- 42× --no-oracle, 7× Runde 5 -- exakter Alpha-Beta-Solver (round5.rs), nicht netz-oracle-bewertet
- KI: nicht bewertete Züge -- 49× --no-oracle, 9× Runde 5 -- exakter Alpha-Beta-Solver (round5.rs), nicht netz-oracle-bewertet
...
(Rohpunktestand direkt vor der Endwertung, aus der `# SPIELENDE:`-Kopfzeile: Spieler 1 33 : 48 KI -- fehlende "–"-Werte oben bedeuten lediglich 0 Pkt Strafe in dieser Runde, keine Lücke.)

Endwertung (Wertungsplatten-Bonus):

- **Spieler 1**: +4 Pkt -> Gesamt 37 Pkt
  - ↔️ Horizontale Reihen: 0 Pkt
  - ↗️ Diagonale Reihen: 10 Pkt
- **KI**: +-12 Pkt -> Gesamt 36 Pkt
  - ↔️ Horizontale Reihen: 0 Pkt
  - ↗️ Diagonale Reihen: 0 Pkt

Vor der Endwertung stand es 33 : 48; nach dem Wertungsplatten-Bonus 37 : 36.
