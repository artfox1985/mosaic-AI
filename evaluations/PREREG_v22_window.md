<!-- STATUS: ENTSCHIEDEN | Frage: Wie wird das v22-Trainingsfenster zugeschnitten? | Beleg: ZUSCHNITT ENTSCHIEDEN 2026-08-25 (par.2): 24.000 Partien hv2, eine Klasse, kein Altbestand, ~4,18 Mio Zustaende; Alt-Zuschnitt hinfaellig (par.1). TRAEGERFRAGE par.4: RICHTUNGSENTSCHEID, kein Gating -- Flagge IGNORIEREN (Arm B besser, keine Kennzahl einzeln signifikant; n=40 auf einem Viertelkorpus, par.4a/4b), der volle A/B auf dem ganzen Korpus ist nie gefahren und nie eingetaktet (par.4b). Stand der Kanal-Frage ist par.4e (par.4c/4d ueberholt), naechste Konfiguration Arm B plus OWNERSHIP_WEIGHT>0. par.4f: das DRAFTING traegt es (0,756), das Routing allein nichts (0,000). par.6 Val-Pool UEBERHOLT, v22 als Kaltstart. -->
...
**NACHTRAG 2026-08-27 zur Cache-Begruendung (der Zuschnitt selbst bleibt
unberuehrt):** die 6-KB-Zahl stammt aus der Fensterstrategie vom 2026-08-06,
also vom Tag VOR dem Bitpacking, und die daraus gerechneten ~25 GB waren zu
hoch. **Gemessen am fertigen Cache: 2.811 Byte je Zustand**, fuer 4.186.112
Zustaende also **~11,8 GB** (`PREREG_cache_build_time.md` par.8; auf Platte
0,83 GB mit lzf). Der 24.000er-Entscheid ist damit NICHT nachtraeglich
umzuwerten -- er ist als historischer Entscheid gefallen und bleibt stehen;
falsch war nur die RAM-Obergrenze, mit der er begruendet wurde. Wer die Zahl
kuenftig fuer einen Zuschnitt braucht, nimmt 2.811 B, nicht 6 KB.

**Zweite berichtigte Zahl, 2026-08-28 aus dem Statuskopf hierher verschoben:**
im Kopf dieser Datei standen zwischenzeitlich **14,1 GB** Cache. Auch die war
falsch -- sie war aus der v23-Rechnung geborgt und gilt fuer deren **5,02 Mio**
Zustaende, nicht fuer die 4,18 Mio dieses Fensters. Gueltig ist allein der oben
gemessene Wert (~11,8 GB).
...
**UEBERHOLT (Nutzer-Entscheid 2026-08-27): v22 laeuft als KALTSTART, bei
Bedarf Afterburner** (`PREREG_heuristic_v2_long_rows.md` par.3b.2 Nachtrag).
Der vorbereitete Aufruf unten wird NICHT gefahren -- er traegt zudem zwei
Fehler, die hier festgehalten bleiben, damit ihn niemand kopiert: er laedt
`hv2sanity_best` (das ist **Arm A**; Arm B ist `hv2sanity_ptv`), und ihm
fehlen `MOSAIC_IGNORE_POLICY_TARGET_VALID=1` und `--ownership-weight`.
Der gebaute Mechanismus `MOSAIC_VAL_POOL` bleibt gueltig und greift, falls
der Afterburner als Warmstart auf demselben Korpus laeuft.

**Plan (ueberholt, s.o.):** ein Sanity-Modell (`hv2sanity`) auf dem bis dahin
erzeugten Teil des Korpus, und -- wenn es vernuenftig aussieht -- der volle
v22-Lauf als WARMSTART von diesem Modell.
