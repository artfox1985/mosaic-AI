# Spielanalyse: game_rng_schnitt_probe_0_seed20260814.log

Erzeugt von `tools/analyze_game_log.py` (Commit `a191cd7`), Laufzeit 0s.

- Seed: 20260814, Startspieler: Netz, KI-Spieler: Netz (alphazero_v21_2d_brierbest, 40 Sims)
## (a) Zusammenfassung

Endstand (aus dem Log-Text): **Netz 42 : 35 Heuristik**

Replay-Kreuzvalidierung: jede einzelne erzeugte Log-Zeile (`log_since`) wurde exakt (String-Gleichheit inkl. `[Rn] `-Präfix) gegen die Original-Logdatei geprüft (alle 363 Zeilen bestehen).
...
- Netz: nicht bewertete Züge -- 71× --no-oracle, 8× Runde 5 -- exakter Alpha-Beta-Solver (round5.rs), nicht netz-oracle-bewertet
- Heuristik: nicht bewertete Züge -- 53× --no-oracle, 9× Runde 5 -- exakter Alpha-Beta-Solver (round5.rs), nicht netz-oracle-bewertet
...
(Rohpunktestand direkt vor der Endwertung, aus der `# SPIELENDE:`-Kopfzeile: Netz 42 : 35 Heuristik -- fehlende "–"-Werte oben bedeuten lediglich 0 Pkt Strafe in dieser Runde, keine Lücke.)

Endwertung (Wertungsplatten-Bonus):

- **Netz**: +-6 Pkt -> Gesamt 42 Pkt
  - ↗️ Diagonale Reihen: 0 Pkt
  - ↔️ Horizontale Reihen: 3 Pkt
- **Heuristik**: +-9 Pkt -> Gesamt 35 Pkt
  - ↗️ Diagonale Reihen: 0 Pkt
  - ↔️ Horizontale Reihen: 3 Pkt

Vor der Endwertung stand es 42 : 35; nach dem Wertungsplatten-Bonus 42 : 35.
...
- **Entdeckte Logging-Luecke (KI-Bonuschips)**: der Mensch-Pfad `apply_tiling_chips` (py.rs) loggt "🎫 ... komplettiert Reihe N ...", der KI-Pfad (`ai_tiling_step` -> `TilingStep::Chips` -> `apply_bonus_chips_with`, round_end.rs) tut das NICHT. Betroffen in dieser Partie: R2 Netz Reihe 2, R2 Heuristik Reihe 3, R3 Netz Reihe 4, R3 Heuristik Reihe 4, R5 Netz Reihe 3. Das Replay-Werkzeug erkennt die unvollstaendige Zielreihe und holt die Chip-Komplettierung automatisch nach (ohne die dabei entstehende, im Original fehlende "🎫"-Zeile gegen das Log zu pruefen).
