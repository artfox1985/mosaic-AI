...
// Die Shaping-Schicht liegt seit 2026-08-27 in `shaping.rs` (Schritt A).
use crate::shaping::{apply_ownership_shaping, apply_plate_shaping, apply_scoring_shaping,
                     floor_shaping_delta_ego, long_row_init_delta};
// Weiterhin unter dem alten Pfad erreichbar: `derive_search_seed` ist kein
// Shaping-Begriff, sondern Seed-Ableitung, und referee.rs/py.rs rufen sie
// ueber `net_mcts::`. Ein Re-Export kostet nichts und bricht keine Aufrufstelle.
...
/// K3-Profil `MOSAIC_ENVELOPE_PROFILE` als fuenf Kommazahlen (Runde 1..5,
/// z.B. `1,0.92,0.67,0.33,0`). Fehlend -> `ENVELOPE_PROFILE_DEFAULT`; nicht
/// parsbar oder nicht genau fuenf Werte -> Default plus einmalige stderr-
/// Warnung, kein Panic (Laufzeit-Konfiguration darf keinen Prozess stuerzen,
/// gleiche Disziplin wie `read_f64_env`).
pub(crate) fn read_envelope_profile_env() -> [f64; 5] {
    static WARNED: std::sync::OnceLock<()> = std::sync::OnceLock::new();
    let Ok(raw) = std::env::var("MOSAIC_ENVELOPE_PROFILE") else {
    ...
    WARNED.get_or_init(|| {
        eprintln!(
            "⚠️  MOSAIC_ENVELOPE_PROFILE={raw:?} ignoriert -- erwartet genau 5 Kommazahlen \
...
pub struct SearchConfig {
    ...
    /// K3 (e), `PREREG_geometric_envelope.md` par.8.2: `C_HULL`, Gewicht des
    /// Huellen-Potentials am Netz-Blattwert (`shift = C_HULL * tanh(w_e(r) *
    /// (H0 - H1))`, Nullsumme, geklammert). Default `0.0` = aus, byte-identisch.
    pub envelope_search_c: f64,
    /// K3 (d), par.8.3: `W_TILE`, Punkte je Zellenkosten-Einheit fuer den
    /// Huellen-Gewinn eines Tiling-Abschlusses in Runde 1-4. Default `0.0` =
    /// Bestandspfad (Netz-Stichentscheid unveraendert).
    pub envelope_tiling_w: f64,
    /// K3-Profil `w_e(Runde 1..5)` (par.8.5: aus der Verlaesslichkeit des
    /// Value-Kopfs je Traeger berechnet, Runde 5 immer 0). Default ist die
    /// gemessene b01-Kurve (`envelope::ENVELOPE_PROFILE_DEFAULT`).
    pub envelope_profile: [f64; 5],
    /// K3 (d), par.8.6 (Nutzer 2026-09-03): `W_VAL`, Punkte je vorhergesagtem
    /// Endpunkt fuer den Value-Anteil im Tiling-Score (Marge aus Punkte- und
    /// Gegnerpunkte-Kopf am Endzustand des Kandidaten, Gewicht `1 - w_e`).
    /// Default `0.0` = aus; bei `> 0` entfaellt der multiplikative
    /// Netz-Stichentscheid. Nur Runde 1-4.
    pub envelope_tiling_value_w: f64,
    /// Projektions-Modus des Such-Terms (e) (par.8.7/8.9): 0 Raster, 1
    /// Musterreihen (K3-P), 2 Erreichbarkeit, 3 Ownership. Seit der
    /// Promotion 2026-09-04 ein Spec-Feld (Pflicht), damit eine Spec das
    /// Suchverhalten VOLLSTAENDIG festlegt; Env-Default `MOSAIC_ENVELOPE_PROJECTED`.
    pub envelope_projection_mode: u8,
    /// K3-F (par.8.14, gebaut 2026-09-06): `w_flush`, Gewicht des
    /// Freiraeum-Bonus fuer gebundene Musterreihen, die die Huelle nicht
    /// mehr bedienen koennen (`envelope::row_can_serve_hull`). Nur in den
    /// Modi 1 und 4 wirksam; `0.0` = aus, bitidentisch. Spec-Pflichtfeld je
    /// Seite (`envelope_flush_w`), Env-Default `MOSAIC_ENVELOPE_FLUSH_W`.
    pub envelope_flush_w: f64,
    /// Huellenform des Such-Terms (e) (par.8.15 Teil B, gebaut 2026-09-06):
    /// 1 = Dreieck wie bisher (Default, bitidentisch), 2 = Dreieck plus die
    /// zweite Zelle der Rasterzeile 6 (LINKS (5,1), RECHTS (5,4); 22 Zellen,
    /// Gesamtkosten 62). Wirkt in ALLEN Projektions-Modi; Tiling-Zweig (d)
...
impl SearchConfig {
    ...
    pub fn from_env() -> Self {
        Self {
            ...
            envelope_profile: read_envelope_profile_env(),
            envelope_tiling_value_w: read_f64_env("MOSAIC_ENVELOPE_TILING_VALUE_W", 0.0),
    ...
    pub fn from_spec_file(path: &str) -> Result<Self, String> {
        ...
        let envelope_tiling_w = get_required("envelope_tiling_w")?;
        ...
        // K3-F (par.8.14): dieselbe Pflicht-Regel wie die Felder darueber. Die
        // lebenden models/*.spec.json tragen das Feld seit dem K3-F-Bau; aeltere
        // Wheels lehnen es als unbekanntes Feld ab (gewollt, siehe oben).
        let envelope_flush_w = get_required("envelope_flush_w")?;
        if !(envelope_flush_w >= 0.0) {
            return Err(format!(
                "Spec-Datei {path}: 'envelope_flush_w' muss >= 0 sein, ist {envelope_flush_w}"
            ));
        }
        // K3 par.8.15 Teil B (Huellenform): dieselbe Pflicht-Regel; gueltig sind
        // 1 (Dreieck, Bestand) und 2 (plus zweite Zelle der Zeile 6).
        let envelope_hull_form = {
            let v = get_required("envelope_hull_form")?;
            let max = f64::from(crate::envelope::HULL_FORM_MAX);
            if v.fract() != 0.0 || !(1.0..=max).contains(&v) {
                return Err(format!(
                    "Spec-Datei {path}: 'envelope_hull_form' muss eine ganze Zahl 1..{max} sein \
                     (1 Dreieck, 2 Dreieck plus zweite Zelle der Zeile 6), ist {v}"
                ));
            }
            v as u8
        };
        let envelope_profile = {
            let arr = obj
                ...
                .ok_or_else(|| format!("Spec-Datei {path}: Feld 'envelope_profile' fehlt"))?
                .as_array()
                .ok_or_else(|| format!("Spec-Datei {path}: 'envelope_profile' ist kein Feld"))?;
            if arr.len() != 5 {
                return Err(format!(
                    "Spec-Datei {path}: 'envelope_profile' braucht genau 5 Zahlen (Runde 1..5), hat {}",
                    arr.len()
                ));
            }
            ...
            for (i, v) in arr.iter().enumerate() {
                out[i] = v
                    .as_f64()
                    .ok_or_else(|| format!("Spec-Datei {path}: 'envelope_profile'[{i}] ist keine Zahl"))?;
            }
            out
        };
...
pub(crate) const FLOOR_SHAPING_SCALE: f64 = 50.0;
...
pub(crate) fn floor_penalties(state: &GameState) -> (f64, f64) {
...
fn node_from_net_outputs<R: Rng + ?Sized>(
    ...
    let leaf_value = match ACTIVE_LEAF {
        LeafEval::Net => {
            ...
            // K3 (e), PREREG_geometric_envelope.md par.8.2: Huellen-Potential
            // `C_HULL * tanh(w_e(Runde) * (H(Brett 0) - H(Brett 1)))`, reine
            // Zustandsfunktion, Nullsumme, gleiche Bauform wie Floor- und
            // Langreihen-Term. Bei `C_HULL == 0` (Default) KOMPLETT
            // uebersprungen: byte-identisch. In Runde 5 ist `w_e = 0`
...
mod tests {
    ...
    // ihre Tests sind hier geblieben und gehoeren bei Gelegenheit hinterher.
    use crate::shaping::*;
    ...
    fn search_config_from_spec_file_accepts_hv1() {
        ...
        assert_eq!(cfg.envelope_search_c, 0.0);
        assert_eq!(cfg.envelope_tiling_w, 0.0);
        assert_eq!(cfg.envelope_profile, crate::envelope::ENVELOPE_PROFILE_DEFAULT);
        assert_eq!(cfg.envelope_tiling_value_w, 0.0);
    ...
    /// K3 par.8.15 Teil B: `envelope_hull_form` ist Pflichtfeld und nur 1
    /// (Dreieck) oder 2 (plus zweite Zelle der Zeile 6) gueltig.
    #[test]
    fn search_config_from_spec_file_validates_envelope_hull_form() {
        let dir = std::env::temp_dir();
        ...
        let msg = SearchConfig::from_spec_file(path.to_str().unwrap())
            .expect_err("Spec ohne envelope_hull_form muss scheitern");
        assert!(msg.contains("envelope_hull_form"), "{msg}");
        // 0 und MAX + 1 sind ungueltig.
        for bad in [0u8, crate::envelope::HULL_FORM_MAX + 1] {
            std::fs::write(&path, spec(format!(", \"envelope_hull_form\": {bad}").as_str())).unwrap();
            let msg = SearchConfig::from_spec_file(path.to_str().unwrap())
                .expect_err("Form ausserhalb 1..2 muss scheitern");
            assert!(msg.contains("envelope_hull_form"), "{msg}");
        }
        // 1 und 2 kommen unveraendert an.
        for good in [1u8, 2u8] {
            std::fs::write(&path, spec(format!(", \"envelope_hull_form\": {good}").as_str())).unwrap();
            let cfg = SearchConfig::from_spec_file(path.to_str().unwrap()).expect("1 und 2 sind gueltig");
    ...
    fn search_config_from_spec_file_rejects_short_envelope_profile() {
        ...
        assert!(msg.contains("genau 5 Zahlen"), "{msg}");
    }

    /// K3-Knoepfe aus der Umgebung: Defaults (aus, aus, b01-Profil) und ein
    /// gesetztes Profil.
    #[test]
    fn search_config_from_env_envelope_defaults_and_profile() {
        let _guard = SEARCH_CONFIG_ENV_TEST_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        for n in ["MOSAIC_ENVELOPE_SEARCH_C", "MOSAIC_ENVELOPE_TILING_W", "MOSAIC_ENVELOPE_PROFILE"] {
        ...
        assert_eq!(cfg.envelope_tiling_w, 0.0);
        assert_eq!(cfg.envelope_profile, crate::envelope::ENVELOPE_PROFILE_DEFAULT);
        std::env::set_var("MOSAIC_ENVELOPE_PROFILE", "1, 0.75, 0.5, 0.25, 0");
        assert_eq!(SearchConfig::from_env().envelope_profile, [1.0, 0.75, 0.5, 0.25, 0.0]);
        std::env::set_var("MOSAIC_ENVELOPE_PROFILE", "1,0.5");
        assert_eq!(read_envelope_profile_env(), crate::envelope::ENVELOPE_PROFILE_DEFAULT, "zu kurz -> Default");
        std::env::remove_var("MOSAIC_ENVELOPE_PROFILE");
    }
    ...
    fn set_game_shaping_weight_overrides_default_but_only_on_the_setting_thread() {
        ...
        assert!(checked >= 4, "zu wenige auswertbare Stichproben ({checked}) -- Testaufbau pruefen");
        assert!(changed >= 1, "GAME_WEIGHT-Override veraenderte in keinem Fall die Blattbewertung");
        ...
        assert_eq!(
            before, after,
            "GAME_WEIGHT eines FREMDEN Threads darf den aufrufenden Thread nicht beeinflussen"
        );
    }
