...
///
///   Kriterium        n     Rate   Ereignisse
///   Joker (3)      800   0,4300   344
///   Ecken (5)    3 200   0,2847   911
///   Reihen (0)   4 800   0,0552   265
///   Spalten (1)  4 800   0,0225   108
///   Farbreihen(7)4 800   0,0160    77
///   Diagonalen(2)1 600   0,0025     4   <- zu wenige Ereignisse fuer eine Aussage
///
/// **WARUM DARAUS KEIN alpha ABGELEITET WIRD**: gesucht waere `Mittel(x^alpha) =
/// Rate`. Eine ABGESCHLOSSENE Linie hat x = 1, und `1^alpha = 1` fuer jedes alpha
/// -- also ist `Mittel(x^alpha)` immer mindestens der Anteil der abgeschlossenen
/// Faelle, und das IST die Rate; jeder Teilfall kommt echt positiv dazu. Damit gilt
/// `Mittel(x^alpha) > Rate` fuer JEDES alpha, die Gleichung hat keine Loesung.
/// `(fill/kap)^alpha` kann fuer kein alpha eine kalibrierte Wahrscheinlichkeit sein.
///
/// Konsequenz (Nutzer-Entscheid 2026-08-11): **die Injektion ist der LENKER, der
...
///
/// `alphas[c]` gilt fuer Kriterium `c`. `round_gain` hebt ALLE Exponenten ueber die
/// Runden linear an -- `alpha_c(r) = alphas[c] * (1 + round_gain * (r-1)/4)`, also
/// spaeter steiler. `round_gain = 0` (Default) = keine Rundenabhaengigkeit.
///
...
// ── Musterreihen-Fortschritt (Nutzer-Auftrag 2026-08-11) ────────────────────────
//
// MOTIVATION (geprueft): `player_scoring_features` liest ausschliesslich
// `build_grid(player)`, also nur das Kuppelraster (`scoring.rs:568` --
// `build_grid` iteriert nur `player.dome_grid.dome_slots`, kein Zugriff auf
// `player.pattern_lines`). `wertung_progress`/`wertung_progress_alpha` sind
// deshalb innerhalb EINER Runde fuer JEDEN Drafting-Zug identisch -- ein
// Drafting-Zug aendert `pattern_lines`, nie sofort das Kuppelraster (das
// passiert erst beim Tiling am Rundenende, `round_end.rs`). Dieser Term
// schliesst genau diese Luecke: er liest `pattern_lines` UND das Kuppelraster
// gemeinsam, kann also zwei Bretter mit identischem Raster aber
// unterschiedlichen Musterreihen unterscheiden (siehe Test
// `musterreihen_fortschritt_distinguishes_states_wertung_progress_alpha_
// cannot`).
//
// GEOMETRIE (geprueft, siehe Bericht fuer Datei:Zeile): Musterreihe `r`
// (0..6, Kapazitaet `r+1`, `board.rs::PatternLine::capacity`) speist
// AUSSCHLIESSLICH Rasterreihe `r` (`DomeGrid::cell_to_dome_space`:
// `(row6/2, col6/2, (row6%2)*2+(col6%2))` -- der Rasterreihen-Index `row6`
// geht unveraendert in die Slot-/Space-Adressierung ein, exakt der Bezug, den
// `round_end::row_has_open_matching_slot` fuer die Tiling-Platzierbarkeit
// nutzt: `dome_row = row_idx/2`, `space_row = row_idx%2`).

/// Bereitschaft einer LEEREN 6×6-Zelle `(row6, col6)`: wie weit die dafuer
/// zustaendige Musterreihe (== `row6`) schon gefuellt ist, WENN sie fuer diese
/// Zelle ueberhaupt in Frage kommt. 0.0, wenn die Kuppelplatte an dieser
/// Stelle fehlt, das Feld gesperrt ist, die Musterreihe leer ist, oder das
/// Feld die Musterreihenfarbe nicht annimmt. `DomeSpace::accepts` deckt
/// "bereits gefuellt"/"gesperrt"/"Farbe passt nicht" bereits selbst ab
/// (`dome.rs:62-71`); die explizite Reihenfolge hier folgt trotzdem 1:1 der
/// Aufgabenstellung, damit jeder Ausschlussgrund einzeln benannt ist.
/// Degradiert sicher (liefert 0.0), falls doch auf einer bereits gefuellten
/// Zelle aufgerufen -- nicht auf dieses Verhalten angewiesen, siehe Aufrufer.
fn bereitschaft(player: &PlayerBoard, row6: usize, col6: usize) -> f64 {
    let Some(sp) = player.dome_grid.get_space(row6, col6) else {
        return 0.0; // Slot fehlt.
    };
    if sp.is_locked {
        return 0.0;
    }
    let row = &player.pattern_lines[row6];
    let Some(color) = row.color else {
        return 0.0; // Musterreihe leer.
    };
    if !sp.accepts(color) {
        return 0.0; // Feld nimmt die Musterreihenfarbe nicht an.
    }
    row.tiles.len() as f64 / row.capacity() as f64
}

/// Weicher Fuellstand einer Geometrie-Instanz (eine Menge von 6×6-Zellen, die
/// ZUSAMMEN eine Reihe/Spalte/Diagonale/Eckplatte/o.ae. bilden): gefuellte
/// Zellen zaehlen `1.0`, leere Zellen tragen `bereitschaft` bei.
/// PFLICHT-SCHRANKE: je Rasterreihe (== Musterreihe) hoechstens `1.0` --
/// bei mehreren leeren Zellen derselben Rasterreihe INNERHALB der Instanz
/// wird das MAXIMUM genommen, nicht die Summe (eine Musterreihe liefert
/// hoechstens eine Fliese pro Runde, siehe Test
/// `musterreihen_fortschritt_caps_contribution_per_pattern_row`).
fn soft_fill_for_cells(player: &PlayerBoard, cells: &[(usize, usize)]) -> f64 {
    let mut filled = 0.0;
    let mut readiness_per_row = [0.0f64; 6];
    for &(r, c) in cells {
        let Some(sp) = player.dome_grid.get_space(r, c) else {
            continue; // Slot fehlt -- traegt weder zu gefuellt noch zu Bereitschaft bei.
        };
        if sp.is_filled() {
            filled += 1.0;
            continue;
        }
        let ready = bereitschaft(player, r, c);
        if ready > readiness_per_row[r] {
            readiness_per_row[r] = ready;
        }
    }
    filled + readiness_per_row.iter().sum::<f64>()
}

/// Alle 6×6-Zellpositionen der Wildcard-Spaces (fuer Kriterium 3 -- die
/// Analogie zu `collect_spaces(player, SpaceType::Wild)`, aber mit
/// Zellposition statt nur der Space-Referenz).
fn wild_cell_positions(player: &PlayerBoard) -> Vec<(usize, usize)> {
            ...
            if let Some(slot) = &player.dome_grid.dome_slots[sr][sc] {
                for (si, sp) in slot.spaces.iter().enumerate() {
                    if sp.space_type == SpaceType::Wild {
                        cells.push((sr * 2 + si / 2, sc * 2 + si % 2));
                    }
                }
            }
        }
...
/// Weicher Ersatz fuer `row_unique_colors` (Kriterium 7, Farbenreiche
/// Reihen). JUDGMENT CALL (von der Aufgabenstellung nicht vorgegeben, siehe
/// Bericht): die Aufgabe definiert `bereitschaft`/"weichen Fuellstand" nur
/// fuer FUELLSTAND (Belegungszaehler), Kriterium 7 zaehlt aber VERSCHIEDENE
/// FARBEN, ein anderer Begriff. Da Rasterreihe `r` ausschliesslich von
/// Musterreihe `r` gespeist wird und eine Musterreihe stets nur EINE Farbe
/// traegt, kann sie die Farbenvielfalt einer Reihe um HOECHSTENS EINE
/// (neue) Farbe erhoehen -- unabhaengig davon, wie viele Zellen sie am Ende
/// fuellt. Traegt die Musterreihenfarbe schon zu den belegten Farben der
/// Reihe bei, gibt es keinen Zuwachs (0.0); sonst die maximale Bereitschaft
/// unter den passenden leeren Zellen dieser Reihe (degeneriert zu einem
/// einzelnen Maximum, weil alle Zellen einer Reihe per Konstruktion zur
/// selben Musterreihe gehoeren).
fn soft_row_colors(player: &PlayerBoard, row6: usize) -> f64 {
    let hard = row_unique_colors(player, row6) as f64;
    let row = &player.pattern_lines[row6];
    let Some(color) = row.color else {
        return hard;
    };
    let already_present = (0..6).any(|c| {
        player.dome_grid.get_space(row6, c).map_or(false, |sp| {
            !sp.placed_special && sp.placed_color == Some(color)
        })
    });
    if already_present {
        return hard;
    }
    let ready = (0..6).map(|c| bereitschaft(player, row6, c)).fold(0.0f64, f64::max);
    hard + ready
}

/// Spiegelt `wertung_progress_alpha` in Aufbau und Geometrie-Aufzaehlung,
/// ersetzt aber den harten Fuellstand jeder Geometrie-Instanz durch den
/// weichen `soft_fill_for_cells` (siehe dortige Doku fuer die PFLICHT-
/// SCHRANKE je Rasterreihe). Kriterium 4 (Randfelder) bleibt wie im Original
/// LINEAR (kein Exponent, keine Normierung -- zahlt pro Feld, siehe
/// `wertung_progress_alpha`-Doku "Kriterium 4 ... bleiben LINEAR"). Kriterium
/// 6 (Spezialfelder) traegt bewusst NICHTS bei (`0.0`) -- das haelt
/// `unlock_progress_beta` bereits (derselbe Grund wie in
/// `wertung_progress_alpha`: sonst Doppelzaehlung, wenn beide Additive aktiv
/// waeren). `alphas[c]` gilt fuer Kriterium `c`, gleiche Bedeutung wie in
/// `wertung_progress_per_kriterium`.
pub fn musterreihen_fortschritt(player: &PlayerBoard, tile_ids: &[usize], alphas: &[f64; 8]) -> f64 {
    let mut total = 0.0;
    for &id in tile_ids {
        total += match id {
            0 => (0..6)
                .map(|r| {
                    let cells: Vec<(usize, usize)> = (0..6).map(|c| (r, c)).collect();
                    (soft_fill_for_cells(player, &cells) / 6.0).powf(alphas[0])
                })
                .sum::<f64>()
                * 3.0,
            1 => (0..6)
                .map(|c| {
                    let cells: Vec<(usize, usize)> = (0..6).map(|r| (r, c)).collect();
                    (soft_fill_for_cells(player, &cells) / 6.0).powf(alphas[1])
                })
                .sum::<f64>()
                * 7.0,
            2 => {
                let diag0: Vec<(usize, usize)> = (0..6).map(|i| (i, i)).collect();
                let diag1: Vec<(usize, usize)> = (0..6).map(|i| (i, 5 - i)).collect();
                ((soft_fill_for_cells(player, &diag0) / 6.0).powf(alphas[2])
                    + (soft_fill_for_cells(player, &diag1) / 6.0).powf(alphas[2]))
                    * 10.0
            }
            3 => {
                let wild_cells = wild_cell_positions(player);
                let total_w = wild_cells.len() as f64;
                (soft_fill_for_cells(player, &wild_cells) / total_w.max(1.0)).powf(alphas[3]) * 2.0 * total_w
            }
            4 => {
                let cells: Vec<(usize, usize)> = (0..6)
                    ...
                    .collect();
                soft_fill_for_cells(player, &cells)
            }
            5 => {
                let corners = [(0usize, 0usize), (0, 2), (2, 0), (2, 2)];
                let weights = [3.0, 3.0, 8.0, 8.0];
                corners
                    .iter()
                    .zip(weights.iter())
                    .map(|(&(sr, sc), &w)| {
                        let cells: Vec<(usize, usize)> = (0..2)
                            .flat_map(|dr| (0..2).map(move |dc| (sr * 2 + dr, sc * 2 + dc)))
                            .collect();
                        (soft_fill_for_cells(player, &cells) / 4.0).powf(alphas[5]) * w
                    })
                    .sum::<f64>()
            }
            // Spezialfelder: haelt `unlock_progress_beta`, siehe Modul-Doku oben.
            6 => 0.0,
            7 => (0..6)
                .map(|r| (soft_row_colors(player, r) / 5.0).powf(alphas[7]))
                .sum::<f64>()
                * 4.0,
            _ => 0.0,
        };
...
mod tests {
    ...
    fn report_reference_run(label: &str, games: usize, boards: &[PlayerBoard]) {
        ...
        //  (a) Kriterium 3 (Jokerfelder) und 7 (Farbreihen) fehlten -- sie haben
        //      keine "fill/kap"-Array-Form und waren durchgerutscht; ich hatte
        //      sie stillschweigend auf den Spaltenwert gesetzt.
        //  (b) alpha wird jetzt aus der VERTEILUNG gefittet: gesucht ist alpha
        //      mit `Mittel(x^alpha) = Rate`. Die erste Fassung loeste
        //      `(typisches x)^alpha = Rate` -- falsch, weil `E[x^a] != (E[x])^a`
        //      und `x^a` konvex ist, also vom oberen Ende dominiert wird.
        // Ereigniszahl wird MITBERICHTET, weil sie die Belastbarkeit bestimmt
        // (die Diagonale hatte im ersten Lauf ~1 Ereignis).
        println!("  -- Kalibrierung: alpha aus der Verteilung gefittet --");
        println!("       Kriterium      n     Rate  Ereign.  (x)^2   alpha");
        let fit = |obs: &[(f64, bool)]| -> (f64, f64, usize, f64) {
            let n = obs.len() as f64;
            let hits = obs.iter().filter(|(_, h)| *h).count();
            let rate = hits as f64 / n;
            let p2 = obs.iter().map(|(x, _)| x * x).sum::<f64>() / n;
            if hits == 0 || rate >= 1.0 {
                return (rate, p2, hits, f64::NAN);
            }
            // Bisektion: Mittel(x^a) ist monoton FALLEND in a (x in [0,1]).
            let mean_pow = |a: f64| obs.iter().map(|(x, _)| x.powf(a)).sum::<f64>() / n;
            let (mut lo, mut hi) = (0.05f64, 60.0f64);
            for _ in 0..200 {
                let mid = 0.5 * (lo + hi);
                if mean_pow(mid) > rate { lo = mid } else { hi = mid }
            }
        ...
        let mut o: Vec<(f64, bool)> = Vec::new();
        for s in feats.iter() { for &v in s.row_fill.iter() { o.push((v as f64 / 6.0, v >= 6)) } }
        zeile("Reihen(0)", &o); o.clear();
        for s in feats.iter() { for &v in s.col_fill.iter() { o.push((v as f64 / 6.0, v >= 6)) } }
        zeile("Spalten(1)", &o); o.clear();
        for s in feats.iter() { for &v in s.diag_fill.iter() { o.push((v as f64 / 6.0, v >= 6)) } }
        zeile("Diagonalen(2)", &o); o.clear();
        // Kriterium 3: EIN Verhaeltnis je Brett, Treffer = alle Jokerfelder belegt.
        for s in feats.iter() {
            let t = s.wild_total.max(1) as f64;
            o.push((s.wild_filled as f64 / t, s.wild_total > 0 && s.wild_filled == s.wild_total));
        }
        zeile("Joker(3)", &o); o.clear();
        for s in feats.iter() { for &v in s.corner_fill.iter() { o.push((v as f64 / 4.0, v >= 4)) } }
        zeile("Ecken(5)", &o); o.clear();
        // Kriterium 7: Farben je Reihe, Kapazitaet 5 (>=5 zahlt).
        for s in feats.iter() { for &v in s.row_colors.iter() { o.push((v as f64 / 5.0, v >= 5)) } }
        zeile("Farbreihen(7)", &o);
        println!("  (Kriterien 4 Randfelder und 6 Spezialfelder sind ADDITIV -- kein Exponent.)");
    ...
    /// Lokale Kopie des Randfuellungs-Bauers aus den net_mcts-Tests: EXAKT `n`
    /// gefuellte Zellen in Rasterzeile 0 (alles Rand), damit Kriterium 0 und 4
    /// echte Teilwerte sehen.
    fn board_with_border_fill_local(n: usize) -> PlayerBoard {
        let mut p = PlayerBoard::new(0, "P");
        ...
        for sc in 0..3usize {
            ...
            for (si, sp) in t.spaces.iter_mut().enumerate() {
                if si / 2 != 0 || rest == 0 { continue }
                match sp.space_type {
                    SpaceType::Special => sp.placed_special = true,
                    _ => sp.placed_color = sp.required_color.or(Some(Rot)),
                }
                rest -= 1;
            }
    ...
    // ── Musterreihen-Fortschritt ─────────────────────────────────────────────

    #[test]
    fn musterreihen_fortschritt_caps_contribution_per_pattern_row() {
        // Kriterium 4 (Randfelder, linear, keine Normierung): alle 6 Zellen
        // der Rasterreihe 0 sind Rand. Musterreihe 0 (Kapazitaet 1) ist voll
        // mit Rot -> Bereitschaft 1.0 fuer jede der 6 Zellen. OHNE die
        // PFLICHT-SCHRANKE waeren das 6 * 1.0 = 6.0 (sechsmal von derselben
        // Musterreihe gezaehlt); MIT Schranke (Maximum je Rasterreihe) genau
        // 1.0 -- exakt der wichtigste Teil der Aufgabe.
        let mut p = PlayerBoard::new(0, "P");
        p.pattern_lines[0].color = Some(Rot);
        p.pattern_lines[0].tiles = vec![Rot];
        for sc in 0..3usize {
            let spaces = vec![
            ...
            p.dome_grid.place_dome_tile(DomeTile::new(sc, spaces, 0), 0, sc).unwrap();
        }
        let alphas = [2.0f64; 8];
        let term = musterreihen_fortschritt(&p, &[4], &alphas);
        assert!(
            (term - 1.0).abs() < 1e-9,
            "Schranke griff nicht (erwartet 1.0, nicht 6.0): war {term}"
        );
    }

    /// Ein einzelner leerer Kuppelslot in Slot-Reihe 2 / Spalte 0 (Normal-Rot
    /// an allen 4 Spaces) -- speist die Rasterreihe 5 (Kapazitaet 6).
    fn board_row5_slot() -> PlayerBoard {
        let mut p = PlayerBoard::new(0, "P");
        let spaces = vec![DomeSpace::normal(Rot); 4];
        p.dome_grid.place_dome_tile(DomeTile::new(0, spaces, 0), 2, 0).unwrap();
        p
    }

    #[test]
    fn musterreihen_fortschritt_is_monotonic_in_matching_fill_and_ignores_wrong_color() {
        let alphas = [2.0f64; 8];

        let mut p2 = board_row5_slot();
        p2.pattern_lines[5].color = Some(Rot);
        p2.pattern_lines[5].tiles = vec![Rot; 2];
        let term2 = musterreihen_fortschritt(&p2, &[0], &alphas);

        let mut p3 = board_row5_slot();
        p3.pattern_lines[5].color = Some(Rot);
        p3.pattern_lines[5].tiles = vec![Rot; 3];
        let term3 = musterreihen_fortschritt(&p3, &[0], &alphas);
        assert!(
            term3 > term2,
            "eine Fliese mehr in passender Farbe muss den Term erhoehen: {term2} -> {term3}"
        );

        let term_empty = musterreihen_fortschritt(&board_row5_slot(), &[0], &alphas);

        let mut p_wrong = board_row5_slot();
        p_wrong.pattern_lines[5].color = Some(Blau);
        p_wrong.pattern_lines[5].tiles = vec![Blau; 2];
        let term_wrong = musterreihen_fortschritt(&p_wrong, &[0], &alphas);
        assert!(
            (term_empty - term_wrong).abs() < 1e-12,
            "eine Fliese in einer vom Feld nicht akzeptierten Farbe darf den Term nicht \
             veraendern: {term_empty} vs {term_wrong}"
        );
    }

    #[test]
    fn musterreihen_fortschritt_distinguishes_states_wertung_progress_alpha_cannot() {
        // Zwei Zustaende mit IDENTISCHEM Kuppelraster (nichts belegt), die
        // sich NUR in den Musterreihen unterscheiden -- genau der Fall, den
        // `wertung_progress`/`wertung_progress_alpha` per Konstruktion NICHT
        // sehen kann (liest ausschliesslich `build_grid`, siehe Modul-Doku
        // bei `musterreihen_fortschritt`).
        fn board(fill_line5: bool) -> PlayerBoard {
            let mut p = board_row5_slot();
            if fill_line5 {
                p.pattern_lines[5].color = Some(Rot);
                p.pattern_lines[5].tiles = vec![Rot; 3];
            }
            p
        }
        let a = board(false);
        ...
        // Beleg: wertung_progress_alpha sieht keinen Unterschied.
        let alpha_a = wertung_progress_alpha(&a, &[0], 2.0);
        let alpha_b = wertung_progress_alpha(&b, &[0], 2.0);
        assert_eq!(alpha_a, alpha_b, "wertung_progress_alpha darf Musterreihen nicht sehen koennen");

        // Der neue Term sieht den Unterschied -- das ist der eigentliche Zweck.
        let alphas = [2.0f64; 8];
        let m_a = musterreihen_fortschritt(&a, &[0], &alphas);
        let m_b = musterreihen_fortschritt(&b, &[0], &alphas);
        assert_eq!(m_a, 0.0, "leere Musterreihe -> kein Fortschritt");
        assert!(m_b > 0.0, "gefuellte, passende Musterreihe muss Fortschritt zeigen");
        assert!(m_a != m_b, "musterreihen_fortschritt muss die Drafting-Zustaende unterscheiden");
    }
