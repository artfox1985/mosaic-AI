...
pub const KNOBS: &[KnobEntry] = &[
    ...
    KnobEntry { name: "MOSAIC_ACTION_TEMP", default: "0.0 (= aus, rohe Besuchszahlen)", status: KnobStatus::Diagnose, purpose: "Arme S2/S5 der v25-Sockelstruktur: aktionsabhaengige Temperatur fuer die ZUGWAHL im Netz-Self-Play. MODUS, kein Faktor: 1 = Staffel wie im Heuristik-Pfad (n>50 -> 0,7; n>15 -> 0,4; sonst 0,15), 2 = glatte Form (par.14d, Nutzer 2026-09-07): logarithmisch von T 0,2 bei 2 Aktionen bis 0,8 bei 64, ausserhalb gekappt -- die Anker sind gemessen (Median 4, neuntes Dezil 64 aus 6.602 Entscheiden). Gewichte werden visits^(1/T) statt visits; das Policy-ZIEL (completed-Q) bleibt unberuehrt. Bei 0 exakt die rohen Besuchszahlen, bitidentisch (net_mcts::action_temp_mode, self_play::action_temp_weights)", prereg: "PREREG_v25_window.md par.14, par.14d" },
    KnobEntry { name: "MOSAIC_DEVIATE_PROB", default: "0.0 (= aus)", status: KnobStatus::Diagnose, purpose: "Weg C (KataGo, Wu 2019 Anhang D): Wahrscheinlichkeit je Self-Play-Partie, dass GENAU EIN Drafting-Zug von der Suche abweicht; KataGo faehrt 0,05. Bei 0.0 wird keine einzige Zusatz-Zufallszahl gezogen (self_play.rs, deviate_prob/deviation_move)", prereg: "PREREG_start_position_seeding.md par.9c" },
    KnobEntry { name: "MOSAIC_DEVIATE_MEAN_MOVE", default: "30.0", status: KnobStatus::Diagnose, purpose: "Weg C: Mittelwert der Exponentialverteilung, aus der die Halbzugnummer der Abweichung gezogen wird (gezaehlt wie move_number: echte Drafting-Entscheide, 1-basiert, beide Spieler zusammen) (self_play.rs, deviate_mean_move)", prereg: "PREREG_start_position_seeding.md par.9c" },
    KnobEntry { name: "MOSAIC_DEVIATE_CANDIDATES", default: "6", status: KnobStatus::Diagnose, purpose: "Weg C: wie viele legale Aktionen an der Abweichungsstelle gleichverteilt gezogen werden, jede mit EINER Netzbewertung ihres Folgezustands gefiltert; KataGo zieht 3 bis 10, wir nehmen einen festen Wert (self_play.rs, deviate_candidates/deviation_candidates_from)", prereg: "PREREG_start_position_seeding.md par.9c" },
    KnobEntry { name: "MOSAIC_EXCURSION_PROB", default: "0.0 (= aus)", status: KnobStatus::Diagnose, purpose: "Weg B: Wahrscheinlichkeit je Self-Play-Partie, dass ein Ausflug entsteht -- eine ZWEITE, vollstaendige Partie ab einer per gewichtetem Reservoir gezogenen Abzweigstelle (Gewicht = Rundenprofil * Aktionszahl), die Hauptpartie laeuft unbeirrt weiter. Bei 0.0 wird keine einzige Zusatz-Zufallszahl gezogen und kein Zustand geklont (self_play.rs, excursion_prob/excursion_gate/reservoir_step)", prereg: "PREREG_start_position_seeding.md par.9f" },
    KnobEntry { name: "MOSAIC_EXCURSION_TAU_MOVES", default: "12", status: KnobStatus::Diagnose, purpose: "Weg B: wie viele Halbzuege der Ausflug AB SEINEM EIGENEN START sampelt, bevor er greedy wird (RELATIV, nicht absolut -- eigener move_number-Zaehler ab 1); unabhaengig vom globalen MOSAIC_TAU_ARGMAX_FROM_MOVE (self_play.rs, excursion_tau_moves, NetSelfPlayAgent::tau_argmax_override)", prereg: "PREREG_start_position_seeding.md par.9f" },
