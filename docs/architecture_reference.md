...
| `tiling_solver.rs::tiling_key` (2026-09-21) | Der Cache-Schluessel des Tiling-Solvers fuehrt je Bonuschip nur noch die Farb-MENGE als Bitmaske (`round_end::chip_sig`) statt der Farbliste; die Reihenfolge INNERHALB eines Chips faellt damit weg | in Ordnung. **Wessen Informationsmenge:** keine -- der Schluessel ist kein Spielzustand, sondern eine Memoisierung ueber `state.players[pi]`. **Was weggenommen wird:** nichts, was ein Spieler hat. Die Reihenfolge in einem Chip ist ein Artefakt der Abschrift (`docs/bonus_chips_colors.csv` fuehrt zwei von fuenf zweifarbigen Kombinationen verdreht), im Spiel bedeutungslos; Wertung und Verbrauch lesen sie ohnehin als Menge (`round_end::chip_sig`, `colors.contains`). **Was ausdruecklich NICHT weggenommen wird:** die Reihenfolge UEBER die Chips. Ein erster Anlauf am 2026-09-20 sortierte auch die, mit derselben Austauschbarkeits-Begruendung -- das ist falsch, weil `chip_allocations` ab mehr als `CHIP_ALLOC_CAP` (14) Chips auf `greedy_chip_indices` zurueckfaellt und dort in HANDINDEX-Reihenfolge waehlt (`round_end.rs:574`, Vorfall `referee.rs:791-795`). Zwei Haende mit derselben Multimenge in anderer Reihenfolge koennen also verschieden ausgehen und duerfen keinen Schluessel teilen. Die Bitmaske deckt zusaetzlich den JSON-Weg ab: `serialize::bonus_chip_from_json` uebernimmt die Farbliste woertlich, Records von vor der Kanonisierung tragen dort weiter verdrehte Paare |
| `dome.rs::build_bonus_chip_pool` (2026-09-20) | sortiert die Farben jedes Plaettchens beim Vorratsbau kanonisch | in Ordnung, gleiche Begruendung wie die Zeile darueber -- das ist die Ursachenbehebung fuer engine-gebaute Zustaende. Sie deckt rekonstruierte Zustaende NICHT ab, dafuer sorgt die Bitmaske |
