#!/usr/bin/env bash
# Fahrplan 36g, Arm v29-b09 (PREREG_minimal_strength_core.md par.10.7): DAS v30-REZEPT AUF DEM v29-FENSTER --
# b03-Rezept mit --moon-loss-weight 0 (b05, moon_stack_order par.12.8), --ownership-weight 0 und OHNE
# --endgame-head (b08, par.10.6); opp_points bleibt. Nutzer 2026-09-17: "v29-b09 nehmen wir ebenfalls mit
# und schauen wie er sich schlaegt." Kandidat fuer die Champion-Kanten neben dem staerksten Einzelarm.
#
# INPUT_SIZE kommt aus config.py (884 mit Abschnitt 17 oder 794 -- Entscheid nach Tor 1 von b07); der
# Fenster-Schluessel des Splits traegt sie, der passende Monolith liegt fuer beide Werte schon
# (790ac07353a6 = 884, 421448d12eb8 = 794), build_cache_incremental findet die Bloecke und mergt nur.
#
# NEBENLAST: Split/Merge sind CPU-Auftraege und warten auf freie CPU; Training (GPU) darf neben EINEM
# CPU-Auftrag laufen; Tor 1 wartet wieder. MOSAIC_CHAIN_NO_WAIT=1 ueberspringt die erste Warteschleife.
set -uo pipefail
cd "$(dirname "$0")/.."
...
warte_frei() {
  echo "########## b09-KETTE WARTET ($1) $(date +%F' '%H:%M:%S)"
  while :; do
...
if [ "${MOSAIC_CHAIN_NO_WAIT:-0}" = "1" ]; then echo "   Warteschleife uebersprungen (MOSAIC_CHAIN_NO_WAIT=1) $(date +%H:%M:%S)"; else warte_frei "Split, Bloecke, Monolith"; fi
echo ""
...
python -X utf8 tools/window_train_split.py --file-list data/window_v29.txt --val-frac 0.05 \
  ...
  --train-list-out data/window_v29_b09_train.txt --val-list-out data/window_v29_b09_val.txt \
  > "$ART/v29_b09_split.txt"
cat "$ART/v29_b09_split.txt"
KEY=$(grep -o "Fenster-Schluessel des Trainingsanteils: [0-9a-f]*" "$ART/v29_b09_split.txt" | awk '{print $NF}')
[ -n "$KEY" ] || { echo "STOPP: kein Schluessel"; exit 13; }
echo "KEY=$KEY"
cmp -s data/window_v29_b09_train.txt data/window_v29_v29-b03_train.txt && echo "   Trainingsliste byte-gleich mit b03" || echo "   WARNUNG: Trainingsliste weicht von b03 ab"
...
python -X utf8 -u tools/build_cache_incremental.py --data-dir data --encoder 2d \
  --value-target-variant nortv --workers 6 --file-list data/window_v29_b09_train.txt \
  --merge-out "$CACHE"
...
echo ""
echo "== 3) Training v29-b09 (v30-Rezept: moon 0, ownership 0, ohne endgame) $(date +%F' '%H:%M:%S)"
python -X utf8 -u train.py --name v29-b09 --load v28-b02_brierbest \
  ...
  --value-target-lambda 0.7 --moon-loss-weight 0 --ownership-head-2d --ownership-weight 0.0 \
  --opp-points-head --destretch-a 0.0051 --destretch-b 1.9269 \
  --select-by-brier --fast-loader --seed $SEED
echo "   Training Exit $? ($(date +%H:%M:%S))"

if [ -f models/alphazero_v29-b09_brierbest.onnx ]; then
  A=models/alphazero_v29-b09_brierbest.onnx
elif [ -f models/alphazero_v29-b09.onnx ]; then
  A=models/alphazero_v29-b09.onnx
  echo "HINWEIS: kein _brierbest -- beste Epoche war die letzte, finales Modell wird genommen"
else
  echo "UEBERSPRUNGEN: kein Modell v29-b09 -- Training gescheitert?"; exit 15
fi
...
for S in 20261220 20261221; do
  OUT="$ART/tor1_v29-b09_vs_b03_s${S}.json"
  echo ""
  echo "===== Seed $S $(date +%H:%M:%S)"
  python -X utf8 -u tools/paired_gating.py \
    --model-a "$A" --spec-a "$SPEC" --model-b "$B" --spec-b "$SPEC" \
    --name-a v29-b09 --name-b v29-b03 \
  ...
  python -X utf8 -u tools/plate_points_from_arena.py "$OUT" --block 5 \
    --out "$ART/plate_points_tor1_b09_s${S}.json"
done

echo ""
echo "########## b09-KETTE FERTIG $(date +%F' '%H:%M:%S)"
echo "   Faellig danach: Manifest-Diff gegen b03 (GENAU ownership_weight, endgame_head, cache_file,"
echo "   Name), Verdikt nach par.10.7, sechs Kennzahlen, Bauzeit nach docs/measured_runtimes.md."
