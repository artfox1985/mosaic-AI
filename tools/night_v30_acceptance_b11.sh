#!/usr/bin/env bash
# ABNAHME des 414/888-Wheels UND der Generator-Identitaet v29-b11
...
#   das Wheel (Vertrag, Anker, Paritaet), dann das ARTEFAKT v29-b11 (Polsterung
...
# Generator der v30-Erzeugung wird `v29-b09` (884 Eingaenge, 406er-Policy) mit
# auf 414 GEPOLSTERTEM Policy-Kopf, acht Nullzeilen, ohne Training. Weil das ein
# anderes Artefakt ist als `v29-b09_brierbest`, traegt es einen eigenen Namen:
# `v29-b11` (Regel feedback_measured_identity_gets_own_bxx).
#
...
#
# MESSDATEIEN: Stufe 7 erzeugt `data/selfplay_v29-b11-probe_*.pkl`. Das sind
...
SRC=models/alphazero_v29-b09_brierbest.pth
B07=models/alphazero_v29-b09_brierbest.onnx
GEN=v29-b11
B10=models/alphazero_v29-b11.onnx
SPEC=models/v30_generation.spec.json
# Referenz des Kostentors: 12,0 s je Partie -- Tor 1 b09 gegen b03 auf dem
...
echo ""
# b11-VARIANTE (2026-09-18, Nutzer: "weiter mit a und b09"): Champion und Generator sind b09; die Wheel-Tore
# 0-3 und das Kostentor (10.14) sind fuer DIESES Wheel schon gelaufen und werden mit SKIP_WHEEL_GATES=1
# uebersprungen; gefahren werden Polsterung b09 -> v29-b11, Engine-Kontrolle, A/B b11 gegen b09 und die
# Record-Stichprobe. Kostentor-Riss (+35 Prozent) ist per Nutzer-Entscheid hingenommen (STATUS Punkt 24).
if [ "${SKIP_WHEEL_GATES:-0}" != "1" ]; then
...
# 888/414 (Gegenproben im selben Test: 884/406 -> cfd94509f0aab112).
...
echo ""
fi  # Ende Wheel-Tore (uebersprungen: 10.14 gruen fuer dieses Wheel)
echo "== 4) Generator-Artefakt v29-b11 herstellen (Polsterung, kein Training) $(date +%F' '%H:%M:%S)"
...
echo "   Form: v29-b11 gegen sich selbst, $SPEC BEIDSEITS, 2 x 20 Paare = 80 Partien,"
...
python -X utf8 -u - <<'EOF'
import json, sys
REF = 12.0          # docs/measured_runtimes.md Zeile 187, Tor 1 b09 Seed 20261191, exklusiv
SCHWELLE = 25.0
def sjp(p):
...
open("evaluations/artifacts/v30_wheel_kostentor_verdikt.txt", "w", encoding="utf-8").write(
    ...
    "form=v29-b11 gegen sich selbst, models/v30_generation.spec.json beidseits, "
    "2 x 20 Paare, 400 Sims, 10 Threads, --log-games, exklusiv\n"
    f"neu_s{seeds[0]}={neu[0]}\nneu_s{seeds[1]}={neu[1]}\nneu_mittel={mittel}\n"
    f"referenz_s_je_partie={REF}\n"
    "referenz_quelle=docs/measured_runtimes.md Zeile 187 (Tor 1 b09 gegen b03 auf dem "
    "884-Wheel, Seed 20261191, 200 Paare, 10 Threads, --log-games, exklusiv)\n"
...
fi  # Ende Kostentor-Block (uebersprungen: Ergebnis 10.14 gilt architekturgleich fuer b11, ANNAHME)
warte_frei "A/B gepolstert gegen ungepolstert"
echo ""
echo "== 6) A/B: gepolstert (v29-b11) gegen ungepolstert (v29-b09_brierbest) $(date +%F' '%H:%M:%S)"
echo "   A = $B10, B = $B07, Spec $SPEC BEIDSEITS, 400 Sims, 200 Paare, Blockgroesse 5,"
echo "   SPRT alpha=beta=0,001 (also kein praktischer Frueh-Stopp), Seed 20261271."
echo "   LESART: die acht Nullzeilen sind nach dem maskierten log_softmax gleichverteilte,"
echo "   nicht bevorzugte Masse -- fuer die 406 alten Aktionen ist b11 exakt b09. Erwartet ist"
echo "   deshalb Gleichstand. HAELT b11 die 5-Prozentpunkte-Marge (>= 45,0 Prozent Siege),"
...
AB="$ART/ab_v29-b11_vs_b09_s20261271.json"
python -X utf8 -u tools/paired_gating.py \
  --model-a "$B10" --spec-a "$SPEC" --model-b "$B07" --spec-b "$SPEC" \
  --name-a v29-b11 --name-b v29-b09 \
  --sims-a 400 --sims-b 400 --c-puct 1.5 \
  --block-size 5 --max-pairs 200 --sprt-alpha 0.001 --sprt-beta 0.001 \
  --seed 20261271 --threads 10 --log-games --no-promote-winner --out "$AB"
echo "   Exit $? ($(date +%H:%M:%S))"
...
python -X utf8 -u tools/plate_points_from_arena.py "$AB" --block 5 \
  --out "$ART/plate_points_ab_v29-b11_s20261271.json"
...
print(f"   A/B v29-b11 gegen v29-b09: {a} : {b} von {n} Partien = {quote:.1f} Prozent "
...
if quote < 45.0:
    ...
    sys.exit(60)
print(f"   Marge HAELT ({quote:.1f} Prozent >= 45,0): v29-b11 ist der Generator.")
EOF
RC=$?; [ "$RC" = "0" ] || exit "$RC"
...
MOSAIC_STACK_DRAW_RESEARCH=1 python -X utf8 -u self_play.py --mode network --model "$B10" \
  ...
  --threads 11 --chunk 10 --per-file 10 --seed 20260928 \
...
echo "   Freigabe fuer 'MOSAIC_V30_GENERATOR=models/alphazero_v29-b11.onnx"
echo "   MOSAIC_V30_GEN_NAME=v29-b11 bash tools/night_v30_generate.sh' nach Generationswechsel"
...
echo "   4. docs/generation_naming.md: v29-b11 ist belegt (Eintrag steht, Ergebnis nachtragen)."
...
echo "   6. MOSAIC_DATA_EXCLUDE beim Bau des v30-Fensters um selfplay_v29-b11-probe_ erweitern."
