#!/usr/bin/env bash
# Knopf-Messkette am Champion (PREREG_geometric_envelope.md par.8.11 K3-P2, par.8.14 K3-F, par.12b
...
NET="models/alphazero_v23-b01_brierbest.onnx"; CHAMP="models/v23-b01_k3p10.spec.json"; ART="evaluations/artifacts"
procs() {
  local out
...
tick=0
while true; do
  m=$(procs 'night_v24_b06_chai[n]|night_v24_acceptance_chai[n]|champion_edge_b0[5]|paired_gating|paired_arena|self_play\.py|argmax_profile|claude_pla[y]|maturin|cargo '); m=${m:-999}
  [ "$m" = "0" ] && break
...
run_arm() {  # $1 Tag, $2 Spec, $3.. Env-Knoepfe fuer das argmax-Instrument
  ...
  bash tools/argmax_profile.sh "k3-${tag}-v23b01" "$NET" "$@"; echo "   Exit $?"
  echo "== Arm $tag: gepaarte Arena Arm-Spec gegen Champion-Spec $(date +%H:%M:%S)"
  python -X utf8 -u tools/paired_arena_env_ab.py --env-name MOSAIC_ENVELOPE_SEARCH_C --arms 1.0 --control 1.0 --model "$NET" --model-b "$NET" --spec-a "$spec" --spec-b "$CHAMP" --net-sims 400 --n-games 80 --seed 20261014 --log-games --out-prefix "k3${tag}_vs_k3p_first_s14"
  python -X utf8 -u tools/paired_arena_env_ab.py --env-name MOSAIC_ENVELOPE_SEARCH_C --arms 1.0 --control 1.0 --model "$NET" --model-b "$NET" --spec-a "$CHAMP" --spec-b "$spec" --net-sims 400 --n-games 80 --seed 20261014 --log-games --out-prefix "k3${tag}_vs_k3p_second_s14"
  for d in first second; do
    python -X utf8 -u tools/probes/arena_column_probe.py --artifact "$ART/paired_arena_env_k3${tag}_vs_k3p_${d}_s14.json" --out "$ART/columns_k3${tag}_vs_k3p_${d}_s14.json"
  done
  python -X utf8 -u tools/probes/arena_points_probe.py --artifact "$ART/paired_arena_env_k3${tag}_vs_k3p_first_s14.json" "$ART/paired_arena_env_k3${tag}_vs_k3p_second_s14.json" --out "$ART/points_k3${tag}_vs_k3p_s14.json" || echo "   Kuppel-Bonus-Sonde $tag fehlgeschlagen"
