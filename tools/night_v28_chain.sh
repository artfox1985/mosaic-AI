#!/usr/bin/env bash
# v27-Kette: Traeger-Kennzahl, Traeger-Manifest, Fenster, Bloecke, Monolith, Training v28-b01.
# Nach dem Muster von tools/night_v27_chain.sh. Zuschnitt: PREREG_v28_window.md par.1.
# Vorbereitet 2026-09-10 als Vorlage (v28 ausgesetzt); laeuft von selbst an, sobald die Erzeugung durch ist.
#
...
SEED=20260937
G2_SWARM_PATTERN="selfplay_v25-b01-value-excursion_*.pkl"   # oder selfplay_v25-b01-value-tempc_*.pkl (par.2)

erzeugung_fertig() {
  python - <<'PYEOF'
import glob, io, json
try:
    ps = sorted(glob.glob("data/manifest_v27-b01-value-excursion_*.json"))
...
for k in v26-b01-policy v27-b01-policy v27-b01-value-tempc v27-b01-value-excursion; do
  python -X utf8 -u tools/corpus_sanity_check.py data --pattern "selfplay_${k}_*.pkl" --out "$ART/corpus_sanity_${k}.json"
done
# Tor 2a ex post (docs/generation_loop.md, Praezedenz 2026-09-09): der Generator v27-b01 darf
# als Punktschaetzer nicht unter den Vorgaenger fallen. Nur Vorlage, kein Abbruch.
python - <<'PYCHK'
import json, io
v = {}
for k in ("v26-b01-policy", "v27-b01-policy"):
...
a, b = v["v26-b01-policy"], v["v27-b01-policy"]
print(f"TOR 2a ex post: v27-b01 als Generator {b[0]:.3f} (+-{b[1]:.3f}, {b[2]} Seiten) "
      f"gegen v26-b01 {a[0]:.3f} (+-{a[1]:.3f}) -> {'HAELT' if b[0] >= a[0] else 'GERISSEN (Nutzer-Vorlage)'}")
PYCHK

echo "== 2) Traeger-Manifest v27 (580 = 400 neu + 135 G-1 + 45 G-2) $(date +%H:%M:%S)"
python -X utf8 tools/generate_carrier_manifest.py \
  --pattern "selfplay_v25-b01-policy_*.pkl" --n-files 45 --seed $SEED \
  --include-glob "selfplay_v27-b01-policy_*.pkl" \
  --pick "selfplay_v26-b01-policy_*.pkl:135" \
  --out policy_carrier_manifest_v28.json
python - <<'PYEOF'
import json
d = json.load(open('data/policy_carrier_manifest_v28.json', encoding='utf-8'))
f = d['policy_carrier_files']
neu = [x for x in f if x.startswith('selfplay_v27-b01-policy_')]
g1 = [x for x in f if x.startswith('selfplay_v26-b01-policy_')]
g2 = [x for x in f if x.startswith('selfplay_v25-b01-policy_')]
...
python -X utf8 tools/generate_carrier_manifest.py \
  ...
  --list-out data/v25_swarm_pick_v28.txt --out v25_swarm_pick_v28_manifest.json
echo "   gewaehlt: $(grep -vc '^#' data/v25_swarm_pick_v28.txt) (Soll 145)"

echo "== 4) Fensterliste data/window_v28.txt $(date +%H:%M:%S)"
python - <<'PYEOF'
import glob, os
def base(p): return os.path.basename(p)
neu_pol = sorted(base(p) for p in glob.glob('data/selfplay_v27-b01-policy_*.pkl'))
neu_tmp = sorted(base(p) for p in glob.glob('data/selfplay_v27-b01-value-tempc_*.pkl'))
neu_exc = sorted(base(p) for p in glob.glob('data/selfplay_v27-b01-value-excursion_*.pkl'))
g1_pol = sorted(base(p) for p in glob.glob('data/selfplay_v26-b01-policy_*.pkl'))
g1_tmp = sorted(base(p) for p in glob.glob('data/selfplay_v26-b01-value-tempc_*.pkl'))
g1_exc = sorted(base(p) for p in glob.glob('data/selfplay_v26-b01-value-excursion_*.pkl'))
g2_pol = sorted(base(p) for p in glob.glob('data/selfplay_v25-b01-policy_*.pkl'))
g2_val = [l.strip() for l in open('data/v25_swarm_pick_v28.txt', encoding='utf-8')
...
assert (len(g2_pol), len(g2_val)) == (400, 145), zahlen  # G-2-Schwarm: 145 aus der in par.2 gewaehlten Haelfte
...
with open('data/window_v28.txt', 'w', encoding='utf-8', newline='\n') as fh:
    fh.write('# v27-Fenster (PREREG_v28_window.md par.1): 400 Sockel + 400 temperiert + '
             f'{len(neu_exc)} Ausflug (neu) + 400 G-1-policy + {len(g1_tmp)+len(g1_exc)} '
             'G-1-value + 400 G-2-policy + 145 G-2-value (temperiert)\n')
    fh.write('\n'.join(allf) + '\n')
print(f'window_v28.txt: {len(allf)} Dateien (Soll rund 2.946)')
PYEOF
...
echo "== v28-KETTE FERTIG $(date +%F' '%H:%M:%S)"
echo "   v28-b01 ist der Bezug fuer v28-b02 (Variante B, PREREG_v28_window.md par.6)."
