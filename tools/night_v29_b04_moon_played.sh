...
# Gehaertete Wartebedingung: NUR eine klare 0 gilt als frei.
maschine_frei() {
  local n
  n=$(powershell -NoProfile -Command "@(Get-CimInstance Win32_Process | Where-Object { \$_.CommandLine -match '[p]aired_gating\.py|[f]rozen_referee_match\.py|[o]ffline_diagnosis|[t]rain\.py' -and \$_.Name -notmatch 'pwsh|powershell' }).Count" 2>/dev/null | tr -d '\r' | tail -1)
  [ "$n" = "0" ]
}
